...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          hack_cortex_user_profile: this._safeGetItem('hack_cortex_user_profile'),
          hack_cortex_settings: this._safeGetItem('hack_cortex_settings'),
          hack_cortex_profile: this._safeGetItem('hack_cortex_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('hack_cortex_viewed_features'),
          developer_mode: this._safeGetItem('hack_cortex_developer_mode'),
        ...
        ui: {
          // Tab configuration
          hack_cortex_tab_configuration: this._safeGetItem('hack_cortex_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('hack_cortex_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('hack_cortex_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          hack_cortex_read_logs: this._safeGetItem('hack_cortex_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('hack_cortex_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('hack_cortex_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.hack_cortex_tab_configuration) {
        try {
          this._safeSetItem('hack_cortex_tab_configuration', data.ui.hack_cortex_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'hack_cortex_acknowledged_debug_issues',
        'hack_cortex_acknowledged_connection_issue',
        'error_logs',
        'hack_cortex_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('hack_cortex_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
